...
def catalog_weather(catalog, offset):
    ...
    all_catalogs = {
        ...
        "one-web": g.get('db').selectCatalog_OneWeb_JSON,
        "active": g.get('db').selectCatalog_Active_JSON,
        "analyst": g.get('db').selectCatalog_Analyst_JSON,
        "weather-earth": g.get('db').selectCatalog_WeatherEarth_JSON,
        "weather": g.get('db').selectCatalog_Weather_JSON,
        "noaa": g.get('db').selectCatalog_NOAA_JSON,
        "goes": g.get('db').selectCatalog_GOES_JSON,
        "resource": g.get('db').selectCatalog_EarthResource_JSON,
        "sarsat": g.get('db').selectCatalog_SarSat_JSON,
        "disaster-monitoring": g.get('db').selectCatalog_DisasterMonitoring_JSON,
        "tracking-and-data-relay": g.get('db').selectCatalog_TrackingAndDataRelay_JSON,
        "argos": g.get('db').selectCatalog_ARGOS_JSON,
        "system-weather-and-earth-resources": g.get('db').selectCatalog_All_JSON,
        "planet": g.get('db').selectCatalog_Planet_JSON,
        "spire": g.get('db').selectCatalog_Spire_JSON,
        "communications": g.get('db').selectCatalog_Communications_JSON,
        "geo": g.get('db').selectCatalog_GeoSync_JSON,
        "geo-protected-zone": g.get('db').selectCatalog_GeoProtectedZone_JSON,
        "geo-protected-zone-plus": g.get('db').selectCatalog_GeoProtectedZonePlus_JSON,
        "intelsat": g.get('db').selectCatalog_Intelsat_JSON,
        "ses": g.get('db').selectCatalog_SES_JSON,
        "iridium": g.get('db').selectCatalog_Iridium_JSON,
        "iridium-next": g.get('db').selectCatalog_IridiumNext_JSON,
        "orbcomm": g.get('db').selectCatalog_Orbcomm_JSON,
        "globalstar": g.get('db').selectCatalog_Globalstar_JSON,
        "amateur-radio": g.get('db').selectCatalog_AmateurRadio_JSON,
        "experimental": g.get('db').selectCatalog_Experimental_JSON,
        "other-comm": g.get('db').selectCatalog_OtherComm_JSON,
        "satnogs": g.get('db').selectCatalog_SatNOGS_JSON,
        "gorizant": g.get('db').selectCatalog_Gorizont_JSON,
        "raduga": g.get('db').selectCatalog_Raduga_JSON,
        "molniya": g.get('db').selectCatalog_Molniya_JSON,
        "navigation": g.get('db').selectCatalog_Navigation_JSON,
        "gps-ops": g.get('db').selectCatalog_GPSOperational_JSON,
        "glonass-operational": g.get('db').selectCatalog_GLONASSOperational_JSON,
        "galileo": g.get('db').selectCatalog_Galileo_JSON,
        "beidou": g.get('db').selectCatalog_Beidou_JSON,
        "satellite-based-augmentation": g.get('db').selectCatalog_SatelliteBasedAugmentation_JSON,
        "system-navigation": g.get('db').selectCatalog_SystemNavigation_JSON,
        "sbas": g.get('db').selectCatalog_All_JSON,
        "nnss": g.get('db').selectCatalog_All_JSON,
        "nnss-system": g.get('db').selectCatalog_All_JSON,
        "musson": g.get('db').selectCatalog_RussianLEONavigation_JSON,
        "debris": g.get('db').selectCatalog_Debris_JSON,
        "indian-asat-test": g.get('db').selectCatalog_IndianASATTest_JSON,
        "1999-025": g.get('db').selectCatalog_1999_025_JSON,
        "iridium-33": g.get('db').selectCatalog_Iridium_33_JSON,
        "cosmos-2251": g.get('db').selectCatalog_Cosmos_2251_JSON,
