...
def api_request(endpoint, expect_json_response, post_data, get_params):
    ...
    if post_data:
      ...
      r = requests.get(API_BASE_URL + endpoint, params=get_params)
...
def api_get(endpoint, is_json, params):
    ...
    Also prints timing info.
    """
    return api_request(endpoint, is_json, {}, params)
...
def test_object_influence(snapshot):
    api_response = api_get('/object/influence', False, params=TEST_OBJECT_IDENTIFIER)
    snapshot.assert_match(api_response)

def test_object_info(snapshot):
    api_response = api_get('/object/info', False, params=TEST_OBJECT_IDENTIFIER)
    snapshot.assert_match(api_response)
